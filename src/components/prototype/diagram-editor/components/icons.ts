...
export function getPopoularIcons(): string[] {
  var max = 0;
  var min = 10000000;
  icons.icons.forEach((element) => {
    max = Math.max(max, element.popularity);
    min = Math.min(min, element.popularity);
  });
  let popularIcons = icons.icons.filter((icon) => {
    return icon.popularity > (max + min) / 10;
  });
  let popularName: string[] = [];
  popularIcons.forEach((ele) => {
    popularName.push(ele.name);
  });
  return popularName;
}
