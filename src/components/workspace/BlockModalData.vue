<template>
  ...
  <n-grid :x-gap="30" :y-gap="18" :cols="6" style="width:85%">
    <n-grid-item v-for="(item, index) in files.slice((paginationBlock.current - 1) * paginationBlock.pageSize, paginationBlock.current * paginationBlock.pageSize)" :key="index">
      ...
      >
        <template #cover>
          ...
          <n-image
            ...
            preview-disabled
          ></n-image>
          <div v-if="fileType == 12" class="imgBox" v-html="item.fileImage" style="padding:10px;border-radius: 8px; height: 8vw; width: 100%"></div>
