...
import { ToolbarComponent, ToolbarPlugin, fontFamilyDefaultData } from '@aomao/toolbar-vue';
...
export const pluginConfig: { [key: string]: PluginOptions } = {
  ...
  [Fontsize.pluginName]: {
    ...
    filter: (fontSize: string) => {
      return (
        ['12px', '13px', '14px', '15px', '16px', '19px', '22px', '24px', '29px', '32px', '40px', '48px'].indexOf(
          fontSize
        ) > -1
      );
    },
  ...
  [Fontfamily.pluginName]: {
    ...
    filter: (fontfamily: string) => {
      const item = fontFamilyDefaultData.find((item) =>
        fontfamily.split(',').some((name) => item.value.toLowerCase().indexOf(name.replace(/"/, '').toLowerCase()) > -1)
      );
